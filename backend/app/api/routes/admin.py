...
@router.get("/usuarios")
def get_usuarios(current_user: dict = Depends(require_role([2]))):
...
@router.get("/grupos")
def get_grupos(current_user: dict = Depends(require_role([2]))):
    conn = get_db_connection(); cur = conn.cursor()
...
@router.post("/grupos")
def create_grupo(data: GrupoCreate, current_user: dict = Depends(require_role([2]))):
    conn = get_db_connection(); cur = conn.cursor()
...
def update_grupo(grupo_id: str, data: GrupoUpdate, current_user: dict = Depends(require_role([2]))):
    conn = get_db_connection(); cur = conn.cursor()
    updates, values = [], []
...
def delete_grupo(grupo_id: str, current_user: dict = Depends(require_role([2]))):
    conn = get_db_connection(); cur = conn.cursor()
...
def get_especialidades(grupo_academico_id: int = None, current_user: dict = Depends(require_role([2]))):
    conn = get_db_connection(); cur = conn.cursor()
...
def create_especialidad(data: EspecialidadCreate, current_user: dict = Depends(require_role([2]))):
    conn = get_db_connection(); cur = conn.cursor()
...
def update_especialidad(esp_id: int, data: EspecialidadUpdate, current_user: dict = Depends(require_role([2]))):
    conn = get_db_connection(); cur = conn.cursor()
    updates, values = [], []
    if data.nombre: updates.append("nombre = %s"); values.append(data.nombre)
...
def delete_especialidad(esp_id: str, current_user: dict = Depends(require_role([2]))):
    conn = get_db_connection(); cur = conn.cursor()
...
@router.get("/cursos")
def get_cursos(current_user: dict = Depends(require_role([2]))):
    conn = get_db_connection(); cur = conn.cursor()
...
@router.post("/cursos")
def create_curso(data: CursoCreate, current_user: dict = Depends(require_role([2]))):
    conn = get_db_connection(); cur = conn.cursor()
...
def update_curso(curso_id: str, data: CursoUpdate, current_user: dict = Depends(require_role([2]))):
    conn = get_db_connection(); cur = conn.cursor()
    updates, values = [], []
    if data.nombre: updates.append("nombre = %s"); values.append(data.nombre)
    if data.descripcion is not None: updates.append("descripcion = %s"); values.append(data.descripcion)
...
def delete_curso(curso_id: str, current_user: dict = Depends(require_role([2]))):
    conn = get_db_connection(); cur = conn.cursor()
...
@router.get("/temas")
def get_temas(curso_id: int = None, current_user: dict = Depends(require_role([2]))):
    conn = get_db_connection(); cur = conn.cursor()
...
@router.post("/temas")
def create_tema(data: TemaCreate, current_user: dict = Depends(require_role([2]))):
    conn = get_db_connection(); cur = conn.cursor()
...
def update_tema(tema_id: str, data: TemaUpdate, current_user: dict = Depends(require_role([2]))):
    conn = get_db_connection(); cur = conn.cursor()
    updates, values = [], []
    if data.nombre: updates.append("nombre = %s"); values.append(data.nombre)
...
def delete_tema(tema_id: str, current_user: dict = Depends(require_role([2]))):
    conn = get_db_connection(); cur = conn.cursor()
...
def get_preguntas(estado: str = None, tema_id: int = None, asignatura_id: int = None, solo_pendientes: bool = False, current_user: dict = Depends(require_role([2]))):
    try:
...
def create_pregunta(data: PreguntaCreate, current_user: dict = Depends(require_role([2]))):
    conn = get_db_connection(); cur = conn.cursor()
...
def update_pregunta(pregunta_id: int, data: PreguntaUpdate, current_user: dict = Depends(require_role([2]))):
    conn = get_db_connection(); cur = conn.cursor()
    updates, values = [], []
...
def aprobar_pregunta(pregunta_id: int, current_user: dict = Depends(require_role([2]))):
    conn = get_db_connection()
    cur = conn.cursor()
...
def rechazar_pregunta(pregunta_id: int, motivo: str, current_user: dict = Depends(require_role([2]))):
    conn = get_db_connection()
    cur = conn.cursor()
...
def delete_pregunta(pregunta_id: int, current_user: dict = Depends(require_role([2]))):
    conn = get_db_connection()
    cur = conn.cursor()
...
def get_opciones(pregunta_id: int, current_user: dict = Depends(require_role([2]))):
    conn = get_db_connection(); cur = conn.cursor()
...
def create_opcion(pregunta_id: int, data: OpcionCreate, current_user: dict = Depends(require_role([2]))):
    conn = get_db_connection(); cur = conn.cursor()
...
def update_opcion(opcion_id: int, data: OpcionUpdate, current_user: dict = Depends(require_role([2]))):
    conn = get_db_connection(); cur = conn.cursor()
    updates, values = [], []
...
def delete_opcion(opcion_id: int, current_user: dict = Depends(require_role([2]))):
    conn = get_db_connection(); cur = conn.cursor()
...
def get_estadisticas(current_user: dict = Depends(require_role([2]))):
    conn = get_db_connection(); cur = conn.cursor()
...
@router.get("/bloques")
def get_bloques(current_user: dict = Depends(require_role([2]))):
    conn = get_db_connection(); cur = conn.cursor()
...
@router.post("/bloques")
def create_bloque(data: BloqueCreate, current_user: dict = Depends(require_role([2]))):
    conn = get_db_connection(); cur = conn.cursor()
...
def update_bloque(bloque_id: int, data: BloqueUpdate, current_user: dict = Depends(require_role([2]))):
    conn = get_db_connection(); cur = conn.cursor()
    updates, values = [], []
    if data.codigo: updates.append("codigo = %s"); values.append(data.codigo)
    if data.nombre: updates.append("nombre = %s"); values.append(data.nombre)
...
def delete_bloque(bloque_id: int, current_user: dict = Depends(require_role([2]))):
    conn = get_db_connection(); cur = conn.cursor()
    cur.execute("DELETE FROM bloques_tematicos WHERE id = %s", (bloque_id,)); conn.commit(); conn.close()
    return {"message": "Bloque eliminado"}

@router.get("/areas")
def get_areas(bloque_id: int = None, current_user: dict = Depends(require_role([2]))):
    conn = get_db_connection(); cur = conn.cursor()
...
@router.post("/areas")
def create_area(data: AreaCreate, current_user: dict = Depends(require_role([2]))):
    conn = get_db_connection(); cur = conn.cursor()
...
def update_area(area_id: int, data: AreaUpdate, current_user: dict = Depends(require_role([2]))):
    conn = get_db_connection(); cur = conn.cursor()
    updates, values = [], []
...
def delete_area(area_id: int, current_user: dict = Depends(require_role([2]))):
    conn = get_db_connection(); cur = conn.cursor()
...
def get_asignaturas(area_id: int = None, current_user: dict = Depends(require_role([2]))):
    conn = get_db_connection(); cur = conn.cursor()
...
def create_asignatura(data: AsignaturaCreate, current_user: dict = Depends(require_role([2]))):
    conn = get_db_connection(); cur = conn.cursor()
...
def update_asignatura(asignatura_id: int, data: AsignaturaUpdate, current_user: dict = Depends(require_role([2]))):
    conn = get_db_connection(); cur = conn.cursor()
    updates, values = [], []
...
def delete_asignatura(asignatura_id: int, current_user: dict = Depends(require_role([2]))):
    conn = get_db_connection(); cur = conn.cursor()
...
def get_grupos_academicos(current_user: dict = Depends(require_role([2]))):
    conn = get_db_connection(); cur = conn.cursor()
...
def create_grupo_academico(data: GrupoAcadCreate, current_user: dict = Depends(require_role([2]))):
    conn = get_db_connection(); cur = conn.cursor()
...
def update_grupo_academico(grupo_id: int, data: GrupoAcadUpdate, current_user: dict = Depends(require_role([2]))):
    conn = get_db_connection(); cur = conn.cursor()
    updates, values = [], []
    if data.codigo: updates.append("codigo = %s"); values.append(data.codigo)
    if data.nombre: updates.append("nombre = %s"); values.append(data.nombre)
...
def delete_grupo_academico(grupo_id: int, current_user: dict = Depends(require_role([2]))):
    conn = get_db_connection(); cur = conn.cursor()
...
def get_config_puntaje(grupo_id: int = None, current_user: dict = Depends(require_role([2]))):
    conn = get_db_connection(); cur = conn.cursor()
...
def create_config_puntaje(data: ConfigPuntajeCreate, current_user: dict = Depends(require_role([2]))):
    conn = get_db_connection(); cur = conn.cursor()
...
def update_config_puntaje(config_id: int, data: ConfigPuntajeUpdate, current_user: dict = Depends(require_role([2]))):
    conn = get_db_connection(); cur = conn.cursor()
    updates, values = [], []
...
def delete_config_puntaje(config_id: int, current_user: dict = Depends(require_role([2]))):
    conn = get_db_connection(); cur = conn.cursor()
...
@router.get("/reportes")
def get_reportes(current_user: dict = Depends(require_role([2]))):
    conn = get_db_connection()
    cur = conn.cursor()
...
def revisar_reporte(reporte_id: int, resolver: bool, current_user: dict = Depends(require_role([2]))):
    conn = get_db_connection()
    cur = conn.cursor()
