...
from app.core.db import get_odb_connection
router = APIRouter(prefix="/comunidad", tags=["comunidad"])
GEMINI_ API_KEY = "AIzaSyAMfa91FpMEqNgs4rKuP5hX2ypqhB7Ib3Y"
SYSTEM_ PROMPT = """Eres el Tutor Virtual de AKADEMUS. Tu función es resolver dudas académicas de manera pedagógica.
...
REGLA 3 (Tono): Eres motivador, profesional y usas un lenguaje sencillo para estudiantes de secundaria/pré-universitarios."""
...
async def llamar_ gemini(pregunta: str) -> str:
    ...
    import json
    url = f"https://generativelanguage.googleapis.com/v1/models/gemini-2.0-fl ash:generateContent?key={GEMINI_ API_KEY}"
    payload = {
...
async def get_ historial(current_user: dict = Depends(get_current_user)):
