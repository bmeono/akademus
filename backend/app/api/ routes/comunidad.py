...
SYSTEM_ PROMPT = "Eres el Tutor Virtual de AKADEMUS. Resuelve dudas academicas de manera pedagogica."
...
async def llamar_ gemini( pregunta: str) -> str:
    ...
    import json
    url = "https://generativelanguage. googleapis. com/v1/models/ gemini-2.0-fl ash: generateContent? key=" + GEMINI_ API_ KEY
    payload = {"contents": [{"parts": [{"text": SYSTEM_ PROMPT + "\n\ nPregunta:\n" + pregunta}]}], "generationConfig": {"temperature": 0.7, "maxOutputTokens": 2048}}
...
async def hacer_ consulta(req: ConsultaRequest, current_ user: dict = Depends(get_ current_ user)):
    ...
    if not row:
        conn. close()
        raise HTTPException(status_ code=404, detail="Usuario no encontrado")
    creditos = row[0] if row[0] else 0
    if creditos <= 0:
        conn. close()
        raise HTTPException(status_ code=403, detail="Sin creditos disponibles")
    try:
        respuesta = await llamar_ gemini(req. pregunta)
    except Exception as e:
        conn. close()
        raise HTTPException(status_ code=500, detail=str(e))
