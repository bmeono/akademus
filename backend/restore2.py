import psycopg2

conn = psycopg2.connect(
    host="db.czhvprbxvhqpprgaiqjd.supabase.co",
    port=5432,
    user="postgres",
    password="1323Bri@ncisc0",
    database="postgres"
)

with open("akademus_backup.sql", "r", encoding="utf-8") as f:
    content = f.read()

blocks = content.split("CREATE TABLE")
blocks = [b.strip() for b in blocks if b.strip()]

cur = conn.cursor()

# Ya migradas
done = ['areas', 'asignaturas', 'auditoria_accesos', 'bloques_tematicos', 'codigos_otp', 'configuraciones', 'configuraciones_puntaje', 'cursos', 'especialidades', 'flashcard_historial', 'flashcards']

for block in blocks:
    table_name = block.split("(")[0].strip()
    if table_name in done:
        continue
    
    print(f"Migrando: {table_name}")
    
    create_lines = [l for l in block.split("\n") if l.strip().startswith("INSERT") == False and l.strip() != ""]
    create_cmd = "CREATE TABLE " + "\n".join(create_lines).strip().rstrip(");")
    
    try:
        cur.execute("DROP TABLE IF EXISTS " + table_name + " CASCADE")
        conn.commit()
    except:
        pass
    
    try:
        cur.execute(create_cmd + ")")
        conn.commit()
        
        inserts = [l.strip() for l in block.split("\n") if l.strip().startswith("INSERT")]
        count = 0
        for ins in inserts:
            try:
                cur.execute(ins)
                conn.commit()
                count += 1
            except:
                conn.rollback()
        print(f"  {count} filas")
    except Exception as e:
        print(f"  Error: {str(e)[:60]}")
        conn.rollback()

conn.close()
print("Listo!")