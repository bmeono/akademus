...
src = "frontend/ src/ pages/ ComunidadPage. tsx"
dst = "frontend/ src/ pages/ ComunidadPage. tsx"
...
for f in os. listdir("frontend/ src/ pages"):
    if "Comunidad" in f:
        print("File: " + f)
...
export default function ComunidadPage() {{
  ...
  const loadData = async () => {{
    ...
    setError(null);
    try {{
      const [asigRes, credRes] = await Promise.all([
    ...
    try {{
      const histRes = await comunidadAPI. getHistorial();
    ...
    <div className="space-y-6 max-w-3xl mx-auto">
      ...
      <Card>
        <div className="space-y-4">
          <div>
            ...
            <select value={{materia || ''}} onChange={{(e) => setMateria(Number(e.target.value))}}
          ...
          <div>
            ...
            <textarea value={{pregunta}} onChange={{(e) => setPregunta(e.target.value.slice(0, 500))}}
              placeholder="Escribe tu duda aquí..." rows={4} maxLength={500}
          ...
          {{respuesta && <div className="p-4 bg-green-50 border border-green-200 rounded-xl">
            <h3 className="font-semibold text-green-800 mb-2 flex items-center gap-2"><Sparkles className="w-4 h-4" /> Respuesta del Tutor:</h3>
            <p className="text-green-700 whitespace-pre-wrap leading-relaxed">{{respuesta}}</p>
          </div>}}
          <button onClick={{handleEnviar}} disabled={{!pregunta.trim() || !materia || consultando}}
      ...
      {{historial.length > 0 && <div>
        ...
        <div className="space-y-2">
          {{historial.map(item => <div key={{item.id}} className="bg-white border border-slate-200 rounded-xl overflow-hidden">
            <button onClick={{() => toggleExpand(item.id)}} className="w-full p-4 flex items-center justify-between text-left hover:bg-slate-50">
              <div>
            ...
            {{expanded === item.id && <div className="p-4 bg-slate-50 border-t border-slate-200">
