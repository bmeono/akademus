...
// Interceptor — manejo de errores y refresh
...
export const simulacrosAPI = {
  ...
  getCreditos:           ()           => api.get('/simulacros/creditos'),
  ...
  downloadResultadoPDF:  (id: number) => {
    const token = localStorage.getItem('access_token');
    window.open(`${API_URL}/simulacros/${id}/resultado-pdf?token=${token}`, '_blank');
  },
};
